...
const FULL_BINDINGS = {
  ...
  NangoSyncQueue: { url: "https://sqs.example/queue/nango-sync" },
...
describe("checkRequiredResources (lambda runtime)", () => {
  ...
  it("flags non-secret resources (queue, bucket) when the binding itself is missing", () => {
    const { WorkflowStorage: _bucket, NangoSyncQueue: _q1, GithubCloneQueue: _q2, ...rest } = FULL_BINDINGS;
    setResource(rest);

    const summary = mod.checkRequiredResources();
    const missingNames = summary.missing.map((entry) => entry.name);
    expect(missingNames).toEqual(
      expect.arrayContaining(["WorkflowStorage", "NangoSyncQueue", "GithubCloneQueue"]),
    );
    ...
    for (const name of ["WorkflowStorage", "NangoSyncQueue", "GithubCloneQueue"] as const) {
