...
export default $config({
  ...
  async run() {
    ...
    // Nango sync queue must materialize before the queue bridge and Worker
    // because those resources consume its URL/ARN/link directly.
    ...
    // This lets Worker-served Nango sync webhooks enqueue SQS jobs without
    // ambient AWS IAM credentials.
    await import("./infra/queue-bridge");
    // Nango sync CF Workflow Worker (dark-launch alongside SQS+Lambda path).
    // Must materialize before the cloud-web Worker because web-worker.ts adds
    // the NANGO_SYNC_WORKFLOW cross-script workflow binding from this module.
