...
No amount of prompt engineering replaces the right infrastructure. That sounds obvious — until you read two recent pieces from the OpenClaw ecosystem that make the point clearly.
...
The model should own the first list. It should never own the second. Every time a team pushes a runtime concern into the prompt, they get a system that works in demos and fails in production at three in the morning.
...
The thinking is solid. It's just aimed at the wrong layer. Push scheduling, change detection, delivery, state, durability into the runtime, and the prompt can finally focus on the part it's good at: deciding what to do, not remembering what happened.
