...
summary: "Adding one provider to a proactive agent — the honest, production-ready, post-incident-review way — is at minimum a sprint. Doing it for four providers is most of a quarter. We mapped the cost so you don't have to discover it."
date: "2026-05-10"
accent: "lavender"
dropcap: false
---
...
That's the cost when a provider *does* offer webhooks. Many don't — or their webhooks are so limited they might as well not exist.

Google Workspace has push notifications for some resources through the Google Cloud Pub/Sub system, but you need a GCP project, a subscription, and a service account just to receive them. Calendar changes are supported. Google Docs changes are not. If you want to know when a doc was edited, you poll the Drive Activity API.
...
A scheduled sync means running a job on a cron — every minute, every five minutes, every hour — that calls the provider's API, compares the current state to the last known state, and surfaces what changed. It sounds simpler than webhooks. In some ways it's harder.
...
<Callout tone="thought" label="The uncomfortable math">
...
A reactive agent — one that waits for a user to ask it something — doesn't need any of this. It has a simple lifecycle: receive prompt, think, respond. The user is the trigger. The user provides the context.
...
- A provider has an incident and replays twelve hours of webhooks at once. Your queue depth alarm fires. You learn, at 2 AM, that your worker doesn't scale horizontally because of a Redis lock you forgot.
...
None of this is bad engineering — it's just what the work actually looks like when you do it honestly. And it keeps going.

## The watcher as a primitive
...
A proactive agent needs a *watcher:* a single interface that says "something changed in a system you care about" with enough context for the agent to act on it. Whether that change came from a webhook, a streaming API, a polling sync, or a Pub/Sub subscription is an implementation detail the agent shouldn't have to know.

The [three primitives](/posts/three-primitives) we keep coming back to — a clock, a watcher, an inbox — exist because each one represents a class of infrastructure that's genuinely hard to build, genuinely undifferentiated, and genuinely required for an agent to be proactive.

The webhook tax is what happens when you build the watcher from scratch, one provider at a time.
