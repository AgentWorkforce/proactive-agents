...
Charlie's framing draws a clear line between daemons and agents. An agent handles a specific request: "build this feature," "fix this bug." A daemon has a standing role: "keep docs current," "triage new bugs," "fix failing CI." It doesn't need a prompt each time. It wakes when relevant events happen, does its bounded job, and stops. Think of it like a junior team member who owns one specific responsibility, knows exactly where their authority ends, and has a standing invitation to ask for help when they're unsure.
</Callout>
