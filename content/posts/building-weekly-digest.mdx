...
summary: "The weekly-digest agent runs every Saturday, scans four sources for mentions, clusters them by topic, and files a GitHub issue. Here's what worked, what broke, and what production actually looks like."
date: "2026-05-11"
accent: "sage"
dropcap: true
---
...
Some parts of this agent were surprisingly painless. Worth noting, because "everything is hard" isn't true and pretending otherwise is its own kind of dishonesty.
...
The restraint is deliberate. The agent files a curated summary somewhere durable and searchable, and goes quiet. Production agents should be boring. The exciting ones are the ones that break.

<Callout tone="cool" label="The delivery primitive matters">
This maps directly back to the [three primitives](/posts/three-primitives). The clock is cron. The watcher is Brave + Reddit. The inbox is GitHub Issues. Choosing GitHub over Slack changed the agent's behavior more than any prompt tuning did, because it changed how humans interacted with the output.
</Callout>
...
At this price point, the "is this worth running?" question never comes up. The agent costs less per month than the electricity to think about whether to keep it.
...
The weekly-digest agent is not sophisticated. A cron job, four API calls, a dedup file, one LLM call, and a GitHub issue. When it runs, the digest is usually four clusters and a handful of links. Some runs turn up nothing. Both outcomes are fine, because the agent's job is to look so we don't have to, and to stay quiet when there's nothing worth saying.
