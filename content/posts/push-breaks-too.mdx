...
summary: "We've spent five essays explaining why polling fails. Here's where push fails too, because a series that only criticizes one side isn't engineering, it's marketing."
date: "2026-05-11"
accent: "rose"
dropcap: true
---

We have spent the last several posts making a case for push-based, event-driven architectures. Polling is wasteful. Cron loops are fragile. Webhooks deliver change the moment it happens, and agents should wake on events, not timers. All of that is true, and we stand by it.

But a series that only documents the failure modes of the approach you don't like, without acknowledging the failure modes of the approach you do like, is advocacy dressed up as engineering. So here is the other side.

Push architectures break. They break in specific, painful, sometimes spectacular ways. Some of these we've encountered firsthand while building the webhook infrastructure described in [the webhook tax](/posts/the-webhook-tax). Others we know from operating webhook-heavy systems at [Nango](https://nango.dev) and from studying how teams handle them in production.
...
Push gives you transitions in real time but is fragile to provider outages. Poll gives you outcomes on a delay but is resilient to them. Neither gives you both, and pretending otherwise is how you end up with missing data.
</Callout>
...
<PullQuote>
Steady-state operation never exercises the burst path. You discover your scaling bottlenecks during replays, when a provider recovers and sends twelve hours of queued events in under a minute.
</PullQuote>
...
Push architectures are better for agents that need to act on changes as they happen and see what something changed *from*, not just what it is now. That conviction is why the [three primitives](/posts/three-primitives) center on push.

But push is not free. It requires infrastructure for replay, observability for debugging, reconciliation for reliability, and backpressure for resilience. Every one of those is engineering work that a pure polling architecture avoids.

The teams that do well with push are the ones that budget for the failure modes from the start, not the ones who discover them in production.
