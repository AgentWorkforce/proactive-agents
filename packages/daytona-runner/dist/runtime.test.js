...
import { DaytonaRuntime, SnapshotNotFoundError, applyDaytonaAuthEnv, resolveDaytonaAuthCredentials, } from './index.js';
...
describe('DaytonaRuntime shared primitives', () => {
    ...
    it('launchDetached creates through the raw sandbox API and returns before SDK waitUntilStarted', async () => {
        ...
        const daytona = {
            target: 'us',
            sandboxApi: {
                createSandbox: async (params, _organizationId, options) => {
                    createCalls.push({ params, options });
                    return { data: { id: 'sbx-starting', state: 'STARTING' } };
                },
            },
            get: async (id) => fakeSandbox({ id, state: 'STARTING' }),
        };
        const runtime = new DaytonaRuntime({
            daytona: daytona,
            snapshot: 'relay-orchestrator-sdk-test',
    ...
    it('launchDetached throws on snapshot-not-found instead of falling back to a typescript base sandbox', async () => {
        ...
        await assert.rejects(() => runtime.launchDetached({
            name: 'issue-greeter',
            env: { WORKFORCE_AGENT_ID: 'agent-1' },
        }), (err) => {
            assert.ok(err instanceof SnapshotNotFoundError);
            assert.equal(err.snapshot, 'missing-snapshot');
            assert.equal(err.message, "Snapshot not found in Daytona: 'missing-snapshot'. Refusing silent fallback to typescript base — fix DEFAULT_SNAPSHOT or rebuild/publish the snapshot before retrying.");
            return true;
        });
        assert.deepEqual(createCalls, [
            {
    ...
    it('getById attaches an existing sandbox without taking ownership by default', async () => {
        ...
        const runtime = new DaytonaRuntime({ daytona: daytona });
    ...
    it('getById filters by requested sandbox states', async () => {
        ...
        assert.deepEqual(await runtime.getById('sbx-stopped', { states: null }), { id: 'sbx-stopped', state: 'STOPPED' });
    });
    ...
    it('findAllByLabels drains cursor-backed iterators beyond one page-size', async () => {
        const sandboxes = Array.from({ length: 250 }, (_, index) => fakeSandbox({
            id: `sbx-page-${index}`,
            state: 'STOPPED',
            createdAt: '2026-05-31T01:00:00.000Z',
        }));
        ...
        const runtime = new DaytonaRuntime({ daytona: daytona });
        const handles = await runtime.findAllByLabels({ purpose: 'workforce-deploy' }, { states: ['STOPPED'], pageSize: 100, owned: true });
    ...
    it('runScript requires session exec by default but allows explicit one-shot exec', async () => {
        ...
        const runtime = new DaytonaRuntime({ daytona: {} });
        const handle = runtime.attachSandbox(sandbox);
        await assert.rejects(() => runtime.runScript(handle, { command: 'node runner.mjs' }), /session execution is not available/);
        assert.deepEqual(sandbox.commands, []);
        const result = await runtime.runScript(handle, {
            command: 'node runner.mjs',
            useSession: false,
    ...
    it('getScriptLogs falls back to the captured log file when the runAsync snapshot is empty (cloud #1516)', async () => {
        ...
        const tailCmd = sandbox.sessionCommands.find((entry) => typeof entry.req.command === 'string' && entry.req.command.startsWith('tail -c'));
        assert.ok(tailCmd, 'expected a tail -c fallback read');
        assert.equal(tailCmd.req.runAsync, false);
        assert.equal(tailCmd.req.command, "tail -c 262144 '/tmp/.daytona-run-tick-deployment-2.log' 2>/dev/null || true");
    });
    it('startScript scopes log redirection to the async command group', async () => {
        ...
        assert.equal(sandbox.sessionCommands[0].req.command, "{\nnode runner.mjs\n} > '/tmp/.daytona-run-tick-deployment-scoped.log' 2>&1");
    });
    ...
    it('uploadBundle fails before upload when parent directory creation fails', async () => {
        ...
        const runtime = new DaytonaRuntime({ daytona: {} });
        const handle = runtime.attachSandbox(sandbox);
        await assert.rejects(() => runtime.uploadBundle(handle, {
            files: [
                { source: Buffer.from('runner'), destination: '/workspace/runner.mjs' },
            ],
    ...
    it('destroy keeps owned handles registered when remote deletion fails', async () => {
        ...
        const runtime = new DaytonaRuntime({ daytona: daytona });
        const handle = runtime.attachSandbox(sandbox, { owned: true });
...
function fakeSandbox(input) {
    ...
    if (input.supportsSession !== false) {
        process.createSession = async (sessionId) => {
            sessions.push(sessionId);
        };
        process.executeSessionCommand = async (sessionId, req, timeout) => {
        ...
        process.getSessionCommand = async (sessionId, commandId) => {
            polledCommands.push({ sessionId, commandId });
            return input.sessionCommand ?? { id: commandId, command: 'true', exitCode: 0 };
        };
        process.getSessionCommandLogs = async (sessionId, commandId) => {
...
async function* sandboxIterator(sandboxes) {
    for (const sandbox of sandboxes) {
        yield sandbox;
    }
}
async function* pagedSandboxIterator(sandboxes, pageSize) {
...
describe('DaytonaRuntime smoke', { concurrency: false }, () => {
    ...
    it('launches a sandbox, runs node -e, and destroys it', { skip: HAS_DAYTONA ? false : 'DAYTONA_API_KEY is not set', timeout: 120_000 }, async () => {
        ...
        assert.equal(result.exitCode, 0, `expected exitCode 0, got ${result.exitCode}: ${result.output}`);
        assert.match(result.output, /\bok\b/, `expected output to contain "ok", got: ${result.output}`);
    });
});
