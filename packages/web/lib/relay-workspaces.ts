...
type CompiledJoinAccess = {
  ...
  // daemon. Empty of per-path denies — see `hasDenyOverrides`.
  relayAuthTokenScopes: string[];
  // True when the workspace has per-path ignored/readonly overrides that
  // produce broad-grant-plus-deny semantics. Those denies are carried as
  // `workspace:<agent>:…` deny-tags in `tokenScopes` + the durable ACL marker
  // (exact-match keyed), which CANNOT be expressed in the strict 4-segment
  // vocabulary yet (needs a daemon-side semantic `deny:scope:` match). The
  // delegated-token route refuses such workspaces rather than mint a token
  // that would grant write to a readonly path.
...
function buildAclScope(agentName: string, action: "read" | "write", path: string): string {
...
export function compileJoinAccess(
  ...
  for (const scope of effectiveScopes) {
    allowScopes.add(buildAclScope(agentName, scope.action, scope.path));
  }
  ...
  for (const targetPath of collectOverrideTargets(ignoredPatterns, readonlyPatterns)) {
    for (const scope of effectiveScopes) {
      ...
      denyScopes.add(buildAclScope(agentName, scope.action, targetPath));
    }
  }
  ...
  if (tokenScopes.size === 0) {
    tokenScopes.add(buildAclScope(agentName, "read", "/__denied__/*"));
  }
