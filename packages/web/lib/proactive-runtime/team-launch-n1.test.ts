...
import { launchMember, scopesFromRelayfileAccessToken } from "@cloud/core/bootstrap/launch-member.js";
...
import type { LaunchMemberOptions } from "@cloud/core/bootstrap/launch-member.js";
...
describe("team-launch-n1 adapter", () => {
  ...
  it("derives a strict issue root and localRoot from a GitHub issue label payload", () => {
    ...
    expect(memberLocalRootForAssignedRoot(assignedRoot!)).toBe(
      "/home/daytona/workspace/github/repos/AgentWorkforce/cloud/issues/123",
    );
  ...
  it("fails closed when localRoot widens beyond the assigned issue root", () => {
    expect(() => assertTeamLaunchN1ReadSurface({
      ...
      localRoot: "/home/daytona/workspace/github/repos/AgentWorkforce/cloud",
    })).toThrow(/localRoot/);
  });
  ...
  it("launches exactly one member through launchMember and returns no token material", async () => {
    const assignedRoot = "/github/repos/AgentWorkforce/cloud/issues/123";
    const expectedWriteScope = pathScope(assignedRoot);
    const relayAuthApiKey = "org-api-key";
    const mintWorkspacePathScopedRelayfileToken = async () => relayPaToken([expectedWriteScope]);
    const capturedLaunchOptions: unknown[] = [];
    const capturedLaunchMemberOptions: TeamLaunchMemberOptions[] = [];
    const capturedLauncherOptions: unknown[] = [];
    ...
    const result = await dispatchTeamLaunchN1(
      ...
      {
        isEnabled: () => true,
        buildLaunchOptions: async (input) => {
          capturedLaunchOptions.push(input);
        ...
        launchMember: async (options) => {
          capturedLaunchMemberOptions.push(options);
          return launchMember({
            ...(options as LaunchMemberOptions),
            ...
            launchOrchestratorSandbox: async (options) => {
              capturedLauncherOptions.push(options);
              return {
                sandboxId: "sandbox-1",
                runId: "run-1",
                workspaceId: "rw_workspace",
    ...
    expect(capturedLaunchMemberOptions).toHaveLength(1);
    expect(capturedLauncherOptions).toHaveLength(1);
    expect(capturedLaunchMemberOptions[0]).toMatchObject({
      memberName: "cloud-team-issue-n1",
      role: "implementer",
      channel: "team-launch-n1-agent",
      assignedRoot,
      ...
      model: "claude-sonnet-4-6",
      fileType: "typescript",
      workflowFileContent: expect.any(String),
      workflowFileName: "member.ts",
    });
    expect(capturedLaunchMemberOptions[0]).not.toHaveProperty("s3CodeKey");
    expect(capturedLaunchMemberOptions[0]).not.toHaveProperty("paths");
    expect(result).toEqual({
      ...
      assignedRoot,
      localRoot: "/home/daytona/workspace/github/repos/AgentWorkforce/cloud/issues/123",
      writeScopes: [expectedWriteScope],
    });
    expect(JSON.stringify(result)).not.toContain("relay_pa_");
    expect(JSON.stringify(result)).not.toContain(relayAuthApiKey);
    expect(JSON.stringify(capturedLauncherOptions[0])).not.toContain(relayAuthApiKey);
    expect(capturedLauncherOptions[0]).toMatchObject({
      memberHarness: "claude",
      memberModel: "claude-sonnet-4-6",
      codeMountPath: "/home/daytona/workspace/github/repos/AgentWorkforce/cloud/issues/123",
      relayfileMountPaths: [assignedRoot],
      relayfileMemberAccess: {
        token: expect.stringMatching(/^relay_pa_/),
  ...
  it("rejects control-plane tokens returned by launchMember", async () => {
    await expect(dispatchTeamLaunchN1(
      {
        workspaceId: "workspace-1",
        agentId: "agent-1",
      ...
      {
        isEnabled: () => true,
        buildLaunchOptions: async (input) => launchOptions({
          assignedRoot: input.assignedRoot,
          localRoot: input.localRoot,
        }),
        launchMember: async (options) => ({
          ...
          relayfileToken: "relay_ws_control_plane_secret",
          writeScopes: [pathScope(options.assignedRoot)],
        }),
      },
    )).rejects.toThrow(/control-plane token/);
  });
  ...
  it("stands down fail-closed when enabled without injected launch dependencies", async () => {
    const result = await dispatchTeamLaunchN1(
      {
        workspaceId: "workspace-1",
        agentId: "agent-1",
    ...
    expect(result).toEqual({
      status: "skipped",
      reason: "launch-deps-unavailable",
      workspaceId: "workspace-1",
      agentId: "agent-1",
      deliveryId: "delivery-1",
    });
  ...
  it("stands down fail-closed when the real launch-options builder has no relay workspace binding", async () => {
    const appWorkspaceId = "50587328-441d-4acb-b8f3-dbe1b3c5de99";
    mocks.readBoundRelayWorkspaceId.mockResolvedValue(null);
    ...
    const result = await dispatchTeamLaunchN1(
      ...
      {
        isEnabled: () => true,
        buildLaunchOptions: buildTeamLaunchMemberOptions,
        launchMember,
      },
    );
  ...
  it("builds a top-level launchMember config with the intended model and no member token material", async () => {
    ...
    const launchPayload = await buildTeamLaunchPayload(
      ...
      {
        buildCredentialBundleForLaunch: async ({ deliveryId, runId, workflowConfig }) => {
          credentialInputs.push({ deliveryId, runId, workflowConfig });
          return {
            ...credentialBundle(),
            runId,
            workflowConfig,
    ...
    expect(credentialInputs[0]).toMatchObject({
      deliveryId: "delivery-1",
      runId: expect.stringMatching(UUID_RE),
    });
    expect(credentialInputs[0]?.runId).not.toBe("delivery-1");
    expect(launchPayload.launchMember).toMatchObject({
      ...
      model: "claude-sonnet-4-6",
      fileType: "config",
      runId: credentialInputs[0]?.runId,
      credentialBundle: expect.objectContaining({
        workspaceId: "rw_workspace",
        runId: credentialInputs[0]?.runId,
      }),
    });
    ...
    const workflowConfig = JSON.parse(
      (launchPayload.launchMember as { workflowConfig: string }).workflowConfig,
    ) as {
      version?: string;
      name?: string;
      swarm?: { pattern?: string; channel?: string; maxConcurrency?: number };
      agents: Array<{ constraints?: { model?: string } }>;
      workflows: Array<{ steps: Array<{ name?: string; type?: string; command?: string; task?: string }> }>;
    };
    expect(workflowConfig.version).toBe("1.0");
    expect(workflowConfig.name).toBe("cloud-team-issue-n1");
    expect(workflowConfig.swarm).toMatchObject({
      pattern: "pipeline",
      channel: "team-launch-n1-agent",
      maxConcurrency: 1,
    });
    expect(workflowConfig.agents[0]?.constraints?.model).toBe("claude-sonnet-4-6");
    // The run lock must be the FIRST step and deterministic (atomic mkdir):
    // a 524-retry sibling bootstrap in the same sandbox fails this step and
    // aborts before its agent spawns — the in-box leg of the #1820 storm fix.
    expect(workflowConfig.workflows[0]?.steps[0]).toMatchObject({
      name: "acquire-run-lock",
      type: "deterministic",
      command: "mkdir /tmp/.team-launch-run-lock",
    });
    expect(workflowConfig.workflows[0]?.steps[1]?.task).toContain(
      "Run gh auth setup-git, then gh repo clone AgentWorkforce/cloud",
    );
    expect(workflowConfig.workflows[0]?.steps[1]?.task).toContain(
      "create the pull request with gh pr create",
    );
    expect(workflowConfig.workflows[0]?.steps[1]?.task).toContain(
      "for Relayfile reads and writes only",
    );
    expect(() => new WorkflowRunner({ cwd: process.cwd() }).validateConfig(workflowConfig)).not.toThrow();
  });
  ...
  it("uses member roster config from the member's deployed persona spec when present", async () => {
    const launchPayload = await buildTeamLaunchPayload(
      ...
      {
        buildCredentialBundleForLaunch: async ({ runId, workflowConfig }) => ({
          ...credentialBundle(),
          runId,
          workflowConfig,
        }),
    ...
    const workflowConfig = JSON.parse(
      ...
      swarm?: { channel?: string };
      agents: Array<{ name?: string; cli?: string; role?: string; constraints?: { model?: string } }>;
      workflows: Array<{ steps: Array<{ agent?: string }> }>;
    };
    expect(workflowConfig.swarm?.channel).toBe("team-launch-n1-agent");
    expect(workflowConfig.agents[0]).toMatchObject({
      name: "cloud-team-issue-codex-member",
      cli: "codex",
      role: "reviewer",
      constraints: { model: "gpt-5" },
    });
    expect(workflowConfig.workflows[0]?.steps[1]?.agent).toBe("cloud-team-issue-codex-member");
  });
  ...
  it("lets a resolved member persona without a model use the harness default", async () => {
    ...
    expect(workflowConfig.agents[0]?.constraints).toBeUndefined();
  });
  ...
  it("stands down fail-closed when the real launch-options builder lacks launch credentials", async () => {
    const launchMember = vi.fn();
    const result = await dispatchTeamLaunchN1(
      {
        workspaceId: "workspace-1",
