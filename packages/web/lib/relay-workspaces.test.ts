...
describe("compileJoinAccess", () => {
  ...
  it("flags hasDenyOverrides when a readonly override produces a per-path deny", async () => {
    ...
    // The broad write grant intersects the readonly path → a `workspace:`
    // deny-tag is produced, which has no 4-segment representation yet. The
    // delegated-token route refuses these rather than mint an unsafe token.
    expect(access.hasDenyOverrides).toBe(true);
    expect(access.aclPermissions.some((p) => p.startsWith("deny:scope:"))).toBe(true);
  });
});
